...
def test_mode(model_fn, model_diffusion, L, mode=0, refield=32, min_size=256, sf=1, modulo=1, noise_level=0, vec_t=None, \
    ...
    model = partial(model_fn, model_diffusion=model_diffusion, diffusion=diffusion, ddim_sample=False,
...
def test_split_fn(model, L, refield=32, min_size=256, sf=1, modulo=1, noise_level=15, vec_t=None,
    ...
    if h * w <= min_size ** 2:
        ...
        if h * w <= 4 * (min_size ** 2):
            ...
            Es = [test_split_fn(model, Ls[i], refield=refield, min_size=min_size, sf=sf, modulo=modulo,
                                noise_level=noise_level, vec_t=vec_t, model_out_type=model_out_type) for i in range(4)]
...
def test_split(model, L, refield=32, min_size=256, sf=1, modulo=1, noise_level=15, vec_t=None,
               model_out_type='pred_xstart'):
    E = test_split_fn(model, L, refield=refield, min_size=min_size, sf=sf, modulo=modulo, noise_level=noise_level,
                      vec_t=vec_t, model_out_type=model_out_type)
    return E
...
def test_x8(model, L, modulo=1, noise_level=15, vec_t=None, model_out_type='pred_xstart'):
    E_list = [test_pad(model, util.augment_img_tensor(L, mode=i), modulo=modulo, noise_level=noise_level, vec_t=vec_t,
                       model_out_type=model_out_type) for i in range(8)]
...
def test_split_x8(model, L, refield=32, min_size=256, sf=1, modulo=1, noise_level=15, vec_t=None,
                  model_out_type='pred_xstart'):
    E_list = [test_split_fn(model, util.augment_img_tensor(L, mode=i), refield=refield, min_size=min_size, sf=sf,
                            modulo=modulo, noise_level=noise_level, vec_t=vec_t, model_out_type=model_out_type) for i in
              range(8)]
...
def model_fn(x, noise_level, model_diffusion, vec_t=None, model_out_type='pred_xstart', \
             diffusion=None, ddim_sample=False, **model_kwargs):
    # search for the time step t, satisfying noise_level equals to the noise level of the pretrained diffusion model
    alphas_cumprod = torch.tensor(diffusion.alphas_cumprod)
...
def describe_params(model):
    ...
    for name, param in model.state_dict().items():
        if not 'num_batches_tracked' in name:
            ...
            msg += ' | {:>6.3f} | {:>6.3f} | {:>6.3f} | {:>6.3f} || {:s}'.format(v.mean(), v.min(), v.max(), v.std(),
                                                                                 name) + '\n'
    return msg
...
def grad_and_value(operator, x, x0_hat, measurement):
    difference = measurement - operator(x0_hat)
