...
class MetricWrapper(nn.Module):
    def __init__(self):
        ...
        self.lpips_wrapper = LPIPS()
        self.dists_wrapper = DISTS()
        self.msssim_wrapper = MS_SSIM(data_range=1.0, size_average=True, channel=3)

    def forward(self, image_A, image_B):
        B, C, H, W = image_A.shape
        if B == 1:
            mse = torch.mean((image_A - image_B) ** 2).item()
            psnr = 10 * np.log10(1 / mse)
            lpips = self.lpips_wrapper(image_A, image_B, normalize=True).item()
            dists = self.dists_wrapper(image_A, image_B, batch_average=True).item()
            msssim = self.msssim_wrapper(image_A, image_B).item()
            return {
                "psnr": psnr,
                "lpips": lpips,
                "dists": dists,
            ...
            msssim = 0
            for i in range(B):
                mse = torch.mean((image_A[i] - image_B[i]) ** 2).item()
                psnr += 10 * np.log10(1 / mse)
                lpips += self.lpips_wrapper(image_A[i].unsqueeze(0), image_B[i].unsqueeze(0), normalize=True).item()
                dists += self.dists_wrapper(image_A[i].unsqueeze(0), image_B[i].unsqueeze(0), batch_average=True).item()
                msssim += self.msssim_wrapper(image_A[i].unsqueeze(0), image_B[i].unsqueeze(0)).item()
            return {
                "psnr": psnr / B,
                "lpips": lpips / B,
                "dists": dists / B,
...
class DictAverageMeter:
    ...
    def update(self, val):
        ...
        if self.count == 1:
            ...
            for key in val:
                self.sum[key] += val[key]
        self.avg = {key: self.sum[key] / self.count for key in self.sum}
...
def rgb2ycbcr_batch(img_tensor, only_y=True):
    ...
    if only_y:
        ...
        rlt[:, 0, :, :] = (
                0.299 * img_tensor[:, 0, :, :] + 0.587 * img_tensor[:, 1, :, :] + 0.114 * img_tensor[:, 2, :, :])
        rlt[:, 1, :, :] = (
                128.0 - 0.169 * img_tensor[:, 0, :, :] - 0.331 * img_tensor[:, 1, :, :] + 0.5 * img_tensor[:, 2, :,
                                                                                                :])
        rlt[:, 2, :, :] = (
                128.0 + 0.5 * img_tensor[:, 0, :, :] - 0.419 * img_tensor[:, 1, :, :] - 0.081 * img_tensor[:, 2, :,
                                                                                                :])
